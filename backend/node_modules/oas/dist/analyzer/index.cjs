...
function canonicalizeRefPointer($ref) {
  const pointer = _chunkY5HU4SVHcjs.toPointer.call(void 0, $ref);
  try {
...
function resolvePathItemTargetPointer(raw, definition) {
  if (!_chunk7PWF3F2Wcjs.isRef.call(void 0, raw)) {
    return void 0;
  }
  let current = raw;
  let lastRef = raw.$ref;
  const seen = /* @__PURE__ */ new Set();
  while (_chunk7PWF3F2Wcjs.isRef.call(void 0, current)) {
    ...
    let next;
    try {
      next = _jsonpointer2.default.get(definition, canonicalizeRefPointer(ref));
    } catch (e2) {
      break;
    }
    if (next === void 0 || !_chunk7PWF3F2Wcjs.isRef.call(void 0, next)) {
      break;
    }
    current = next;
  }
...
function resolvePathItem(raw, definition) {
  const resolved = _chunkY5HU4SVHcjs.dereferenceRef.call(void 0, raw, definition);
  if (!resolved || typeof resolved !== "object" || _chunk7PWF3F2Wcjs.isRef.call(void 0, resolved)) {
    return void 0;
  }
  return resolved;
}
function resolveMethodKey(container, method) {
  const methodKey = resolveKey(Object.keys(container), method);
  if (!methodKey || !_chunkY5HU4SVHcjs.supportedMethods.includes(methodKey.toLowerCase())) {
    return void 0;
  }
  return methodKey;
}
function accumulateReachableRefs(definition, seeds) {
  ...
  while (queue.length) {
    ...
    let resolved;
    try {
      resolved = _jsonpointer2.default.get(definition, _chunkY5HU4SVHcjs.toPointer.call(void 0, ref));
    } catch (e3) {
      continue;
    }
    if (resolved === void 0) {
    ...
    _chunkY5HU4SVHcjs.collectRefsInSchema.call(void 0, resolved).forEach((nestedRef) => {
...
function buildAnchors(rootPointer, extraPointers, reachableRefs) {
  ...
  reachableRefs.forEach((ref) => anchors.push(_chunkY5HU4SVHcjs.toPointer.call(void 0, ref)));
...
function computeOperationScope(definition, path, method) {
  ...
  const rawPathItem = definition.paths[pathKey];
  ...
  const rootPointer = `/paths/${_chunkY5HU4SVHcjs.encodePointer.call(void 0, pathKey)}/${methodKey}`;
  const extraPointers = [];
  const seeds = new Set(_chunkY5HU4SVHcjs.collectRefsInSchema.call(void 0, operation));
  const targetPointer = resolvePathItemTargetPointer(rawPathItem, definition);
  if (targetPointer) {
    extraPointers.push(`${targetPointer}/${methodKey}`);
  }
  if (pathItem.parameters) {
    extraPointers.push(targetPointer ? `${targetPointer}/parameters` : `/paths/${_chunkY5HU4SVHcjs.encodePointer.call(void 0, pathKey)}/parameters`);
    _chunkY5HU4SVHcjs.collectRefsInSchema.call(void 0, pathItem.parameters).forEach((ref) => seeds.add(ref));
  }
...
function computeWebhookScope(definition, webhookName, method) {
  ...
  const rawWebhook = webhooks2[webhookKey];
  ...
  const rootPointer = `/webhooks/${_chunkY5HU4SVHcjs.encodePointer.call(void 0, webhookKey)}/${methodKey}`;
  const extraPointers = [];
  const seeds = new Set(_chunkY5HU4SVHcjs.collectRefsInSchema.call(void 0, operation));
  ...
  if (webhook.parameters) {
    extraPointers.push(
      targetPointer ? `${targetPointer}/parameters` : `/webhooks/${_chunkY5HU4SVHcjs.encodePointer.call(void 0, webhookKey)}/parameters`
    );
    _chunkY5HU4SVHcjs.collectRefsInSchema.call(void 0, webhook.parameters).forEach((ref) => seeds.add(ref));
  }
  const security = "security" in operation ? operation.security : definition.security;
  collectSecuritySchemeRefs(security).forEach((ref) => seeds.add(ref));
  const reachableRefs = accumulateReachableRefs(definition, seeds);
...
async function buildAnalysis(definition, query2, scope) {
  ...
  if (!query2 || query2.includes("circularRefs")) {
    ...
    const circularRefs = (scope ? dereferencedCircularRefs.filter((ref) => isPointerInScope(_chunkY5HU4SVHcjs.toPointer.call(void 0, ref), scope)) : [...dereferencedCircularRefs]).toSorted();
  ...
  if (!query2 || query2.includes("webhooks")) {
